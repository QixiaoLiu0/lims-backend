...
public class ParameterDAO extends BaseJdbcDao {
    ...
    public List<Parameter> selectParametersByTestTypeId(int testTypeId) throws Exception {
    	String sql = "SELECT parameter_id, test_type_id, parameter_name, unit, `limit`" + "FROM parameter WHERE test_type_id = ?";
    ...
    public void insertParameter(Parameter entity) throws Exception {
    	String sql = "INSERT INTO parameter (test_type_id, parameter_name, unit, `limit`)" + "VALUES (?, ?, ?, ?)";
    ...
    public void updateParameter(Parameter existingEntity) throws Exception {
    	String sql = "UPDATE parameter SET parameter_name = ?, unit = ?, `limit` = ? " + "WHERE parameter_id = ?";
        executeUpdate(sql,
